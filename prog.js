...
$.ajax({
	...
	Object.keys(retData).forEach(function(item, i) {
		Object.keys(retData[item]).forEach(function(item2, i2) {
			let BAIC=Math.floor((retData[item][item2]['BIC']+retData[item][item2]['AIC'])*1000)/1000;
			let PR=Math.floor(retData[item][item2]['pearsonr']*1000)/1000;
			add_html+='<li class="nav-item">';
			add_html+='<a class="nav-link" href="#" id="'+item2+'" onclick="show_model(\''+item+'\',\''+item2+'\')">'+item+' '+PR+' / '+BAIC+'</a>';
			add_html+='</li>';
		});
	});
	add_html+='</ul>';
...
function show_model(name,hash) {
	$(".nav-link").each(function (indx, element) { 
		$(element).attr("class", $(element).attr("class").replace("active", "").trim());
	});
	$("#"+hash).attr("class", $("#"+hash).attr("class")+" active");
	ticker=name;
	trendData=retData[ticker][hash];
	let BIC=Math.floor(trendData['BIC']*1000)/1000;
	let AIC=Math.floor(trendData['AIC']*1000)/1000;
	let PR=Math.floor(trendData['pearsonr']*1000)/1000;
